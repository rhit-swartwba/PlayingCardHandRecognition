...
for typeIdx = 1:length(cardFolders)
    ...
    for bgIdx = 1:length(backgroundImages)
        ...
        %generate image
        [imgOut, data] = GenerateImage(bgImage, images, alphas, idList, style, scalingMin, scalingMax);
        
        % Save image
        outputImageName = fullfile(outputFolder, sprintf('%d_%d.png', typeIdx, bgIdx));
        imwrite(imgOut, outputImageName);
        disp(['Saved combined image: ', outputImageName]);
        
        % Save bounding box
        bboxFileName = fullfile(outputFolder, sprintf('%d_%d.txt', typeIdx, bgIdx));
